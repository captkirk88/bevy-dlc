...
/// Default block size for v5 hybrid format (10 MB). Blocks are individually encrypted tar.gz chunks.
pub const DEFAULT_BLOCK_SIZE: usize = 10 * 1024 * 1024;

/// Arbitrary pack-level metadata stored in v5 containers.
pub type PackMetadata = std::collections::BTreeMap<String, serde_json::Value>;

/// Fully parsed metadata for a `.dlcpack` container.
#[derive(Clone, Debug)]
...
pub const FORBIDDEN_EXTENSIONS: &[&str] = &[
    "dlcpack",
    "pubkey",
    "slicense", // these are the expected extensions for keys/licenses; they should be packed with the dedicated macros rather than as raw files
...
/// V5 manifest entry: binary-serializable format for faster parsing.
...
impl V5ManifestEntry {
    ...
    /// Write this entry in binary format.
    #[allow(dead_code)]
    ...
    /// Read binary format from the pack reader.
    #[allow(dead_code)]
...
/// V5 block metadata: stores information about a tar-gz block within the pack file.
#[derive(Clone, Debug)]
...
// now only supports v5 packs. The old `PackConverter` trait, the
...
/// Pack multiple entries into a single encrypted `.dlcpack` binary using the latest version format.
///
/// This convenience wrapper creates a v5 container with an empty metadata map.
...
/// Pack multiple entries into a v5 `.dlcpack` and include arbitrary pack-level metadata.
...
/// Parse a v5 `.dlcpack` and return the full container metadata.
...
/// Returns: (product, dlc_id, version, entries, block metadata)
