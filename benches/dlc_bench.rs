...
use bevy_dlc::{
    DlcId, DlcPack, EncryptionKey, PackItem, Product, pack_encrypted_pack, parse_encrypted_pack, prelude::DlcPackEntry
};
...
fn bench_pack(c: &mut Criterion) {
    let items = build_items(100, 1024);
...
fn bench_decrypt_all_entries(c: &mut Criterion) {
    ...
    let pack = DlcPack::new(id2, prod2, version.try_into().unwrap(), entries.iter().map(|e| DlcPackEntry::from(e)).collect());
    ...
    bevy_dlc::encrypt_key_registry::register_asset_path(&dlc_id.to_string(), pack_path.to_str().unwrap());
...
criterion_group!(dlc_benches, bench_pack, bench_parse, bench_decrypt_all_entries);
